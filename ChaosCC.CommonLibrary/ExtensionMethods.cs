...
namespace ChaosCC.CommonLibrary
{
    public static class ExtensionMethods
    {
        ...
        {
            if (obj != null && (obj.GetType() == typeof(DataSet) || obj.GetType().BaseType == typeof(DataSet)))
                return ((DataSet)obj).Tables.Count == 0 || ((DataSet)obj).Tables[0].Rows.Count == 0;
            else if (obj != null && obj.GetType() == typeof(DataTable))
                return ((DataTable)obj).Rows.Count == 0;
            else if (obj != null && obj.GetType() == typeof(DataRow[]))
                return ((DataRow[])obj).Length == 0;
        ...
        public static DataTable Paging(this DataTable dataTable, int currentPage = 1, int pageSize = 10)
        {
            if (dataTable.IsNull())
            {
                return dataTable;
            ...
            else
            {
                ...
                {
                    return dataTable.AsEnumerable().Skip(skip).Take(pageSize).CopyToDataTable();
                }
                else
                {
                    DataTable dtNull = dataTable.Clone();
